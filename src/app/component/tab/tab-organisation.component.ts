            ...
            <ui-tabs-menu class="without_buttons">
                <ui-tab [title]="'ГЛАВНАЯ'">
                    ...
                    <ng-container *ngIf="editEnabled">
                        ...
                        ></multiselect-menu>
                        <address-input [block]="organisation?.addressBlock" [addressType]="'office'" [name]="'Адрес'"
                        ...
                        <input-line *ngIf="organisation.goverType != 'franchise' && organisation.goverType != 'main'"
                                      [name]="'Основной офис'" [value]="organisation?.main_office?.name"
                                      (newValue)="organisation.main_office = $event; checkOur($event);"
                                      [query]="{type: 'org', filter:{}}"
                        ></input-line>
                        ...
                        ></multiselect-menu>
                        <input-line [name]="'Контактное лицо'" [value]="organisation?.contact?.name || organisation?.agent?.name || 'Не указано'"
                                    (newValue)="organisation.ourCompany ? organisation.agent = $event : organisation.contact = $event"
                                    [query]="{type: organisation.ourCompany ? 'user' :'person', filter:{}}"
                        ></input-line>
...
export class TabOrganisationComponent implements OnInit, AfterViewInit {
    ...
    agentChanged(event) {
        this.organisation.agentId = event == "null" ? null : event;
    ...
    public delete() {
        this._organisationService.delete(this.organisation).subscribe((stat) =>{
            this._hubService.getProperty("modal-window").showMessage("Организация и все с ней связи удалены");
            this.tab.setEvent({type: 'delete', value: this.organisation.id});
